...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	cp -r ~/.bitway/config/genesis.json $HOMEDIR/config/genesis.json
	...
	$BINARY tx bank send dev1 $($BINARY keys show secp256k1_key -a --home "$HOMEDIR") 2000000000ubtw --chain-id $CHAINID --fees 200ubtw --yes
	sleep 6
	$BINARY keys add segwit_key --key-type segwit --home "$HOMEDIR"
	$BINARY tx bank send dev1 $($BINARY keys show segwit_key -a --home "$HOMEDIR") 2000000000ubtw --chain-id $CHAINID --fees 200ubtw --yes
