...
DENOM_STR="ubtw,sat,uusdc,uusdt,ueth"
INITIAL_ACCOUNT_STR=""
set -f
IFS=,
DENOMS=($DENOM_STR)
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	sed -i.bak "s/bitcoin_rpc_user = \"\"/bitcoin_rpc_user = \"bitway\"/g" $APP_TOML
	...
	$BINARY genesis gentx "${KEYS[0]}" ${INITIAL_SUPPLY%?}${DENOMS[0]} --keyring-backend $KEYRING --chain-id $CHAINID --identity "666AC57CC678BEC4" --website="https://bitway.com" --home "$HOMEDIR"
