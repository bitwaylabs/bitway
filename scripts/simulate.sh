...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  $BINARY tx lending create-pool usdc uusdc --from validator --keyring-backend $KEYRING  --fees 100ubtw --gas auto --chain-id $CHAINID --yes
  sleep 6
  echo "Add liquidity"
  $BINARY tx lending add-liquidity usdc 1000000000uusdc --from validator --keyring-backend $KEYRING  --fees 1000ubtw --gas auto --chain-id $CHAINID --yes
  sleep 6
  echo "Initiail Oracle Prices"
  $BINARY tx lending submit-price 50000 --from validator --keyring-backend $KEYRING --fees 1000ubtw --gas auto --chain-id $CHAINID -y
  sleep 6
  ...
  for i in $(seq 1 $N); do
    echo "\n *** Fund relayer $i ***"
    $BINARY tx bank send validator $($TSS_BIN --home "$HOME/.shuttler$i" address | grep tb1) 10000000ubtw --chain-id $CHAINID --keyring-backend $KEYRING --yes --fees 2000ubtw
    sleep 6
  done
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  \"messages\": [
  {
    \"@type\": \"/bitway.dlc.MsgCreateOracle\",
    \"authority\": \"bitway10d07y265gmmuvt4z0w9aw880jnsr700jwrwlg5\",
    \"participants\": [ ${PARTICIPANTS%?} ],
    \"threshold\": $T
  }
  ],
  \"metadata\": \"\",
  \"deposit\": \"10000000ubtw\",
  ...
  $BINARY tx gov submit-proposal ../build/oracle.json --from validator --fees 1000ubtw --chain-id $CHAINID --keyring-backend $KEYRING -y
  sleep 6
  echo $DCM_PROPOSAL > ../build/dcm.json
  cat ../build/dcm.json
  $BINARY tx gov submit-proposal ../build/dcm.json --from validator --fees 1000ubtw --chain-id $CHAINID --keyring-backend $KEYRING -y
  ...
  $BINARY q gov proposals --output json | jq -r '.proposals| .[] | select(.status == 2) | .id'| while read -r id; do 
    echo "\n   >>>> Vote for $id"; 
    $BINARY tx gov vote $id yes --from validator --fees 1000ubtw --chain-id $CHAINID --keyring-backend $KEYRING -y
    sleep 6;
  done
fi
