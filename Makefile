...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
BITWAY_BINARY = bitwayd
BITWAY_DIR = bitway
BUILDDIR ?= $(CURDIR)/build
HTTPS_GIT := https://github.com/bitwaylabs/bitway.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=bitway \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(BITWAY_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm bitway || true
	# create a new container from the latest image
	$(DOCKER) create --name bitway -t -i ${DOCKER_IMAGE}:latest bitway
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp bitway:/usr/bin/bitwayd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/bitwaylabs/github.com/bitwaylabs/github.com/bitwaylabs/bitway/types"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(BITWAY_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/bitway:Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd testnet init-files --v 4 -o /bitway --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\bitwayd:/bitway\Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
	@docker run --rm -v $(CURDIR)\build\node1\bitwayd:/bitway\Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
	@docker run --rm -v $(CURDIR)\build\node2\bitwayd:/bitway\Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
	@docker run --rm -v $(CURDIR)\build\node3\bitwayd:/bitway\Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
else
	@docker run --rm -v $(CURDIR)/build/node0/bitwayd:/bitway:Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
	@docker run --rm -v $(CURDIR)/build/node1/bitwayd:/bitway:Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
	@docker run --rm -v $(CURDIR)/build/node2/bitwayd:/bitway:Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
	@docker run --rm -v $(CURDIR)/build/node3/bitwayd:/bitway:Z github.com/bitwaylabs/github.com/bitwaylabs/bitway/node "./bitwayd tendermint unsafe-reset-all --home=/bitway"
endif
...
PACKAGE_NAME:=github.com/bitwaylabs/bitway
